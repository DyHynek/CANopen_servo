...
void SDO_Interpolation_period(CANopenNodeSTM32* ____canopenNode, uint8_t ____node_id, uint8_t period)
{
	data[0] = period			& 0xFFU;
	write_SDO(____canopenNode->canOpenStack->SDOclient, ____node_id, 0x60C2, 0x01, data, 1);
}
void SDO_Interpolation_buffer(CANopenNodeSTM32* ____canopenNode, uint8_t ____node_id, int8_t buffer)
{
	data[0] = buffer			& 0xFFU;
	write_SDO(____canopenNode->canOpenStack->SDOclient, ____node_id, 0x60C2, 0x02, data, 1);
}
void SDO_Interpolation_type(CANopenNodeSTM32* ____canopenNode, uint8_t ____node_id, int32_t type)
{
	data[0] = type			& 0xFFU;
	data[1] = (type>>8)		& 0xFFU;
	data[2] = (type>>16)	& 0xFFU;
	data[3] = (type>>24)	& 0xFFU;
	write_SDO(____canopenNode->canOpenStack->SDOclient, ____node_id, 0x60C1, 0x02, data, 4);
}
void SDO_Interpolation_value(CANopenNodeSTM32* ____canopenNode, uint8_t ____node_id, int32_t value)
{
	data[0] = value			& 0xFFU;
	data[1] = (value>>8)		& 0xFFU;
	data[2] = (value>>16)	& 0xFFU;
...
uint8_t SDOmode_display(CANopenNodeSTM32* __canopenNode, uint8_t __node_id, OPERATION_MODE mode)
{
	data[0] = mode	& 0xFFU;
	data[1] = 0x00;
	data[2] = 0x00;
